...
- 📊 **통계 그래프**: 심각도 분포·플랫폼별(도넛)과 ISMS-P 영역별·위치별·MITRE 기법별(가로막대)을 **화면과 PDF 리포트**에 표시합니다. 외부 라이브러리 없이 **순수 SVG**로 그려 폐쇄망·오프라인에서 그대로 동작하고, PDF로 저장해 활용할 수 있습니다.
...
- 🧪 **앱 보안 점검(간이 SAST + WAF)**: 소스코드에서 하드코딩 비밀번호·SQL 인젝션·명령 실행·eval/exec·약한 암호·TLS 검증 해제 등 위험 패턴을 정규식으로 탐지(참고용)하고, WAF(웹 방화벽) 구성이 활성/관리형 룰셋 적용 상태인지 점검.
...
실제 조치 전 대상 환경과 업무 요건을 확인해야 합니다.
