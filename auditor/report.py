...
def format_csv(report: AuditReport) -> str:
    ...
    # 요약 헤더(참고 정보)
    writer.writerow([f"# Azure 정책 오프라인 보안검토 (ISMS-P)  점수 {report.score()}/100  등급 {report.grade()}"])
    counts = report.severity_counts()
    csum = " ".join(f"{k}={counts[k]}" for k in ("CRITICAL", "HIGH", "MEDIUM", "LOW", "INFO") if k in counts)
    writer.writerow([f"# 발견 이슈 {len(report.findings)}건  {csum}"])
    writer.writerow([])  # 빈 줄
    # 열 제목
...
def format_html(report: AuditReport) -> str:
    ...
    for code in sorted(by_code, key=lambda c: [int(p) for p in c.split(".")]):
        ...
        cmd_html = ""
        if ctrl.get("azure_cmd"):
            cmds = "<br>".join("$ " + _esc(c) for c in ctrl["azure_cmd"].splitlines())
            cmd_html = f"<div class='cmd'><b>[참고] 재점검 CLI</b><br>{cmds}</div>"
        sections.append(
            f"<section><h2>[{_esc(code)}] {_esc(ctrl.get('domain',''))}</h2>"
...
<title>Azure 정책 오프라인 보안검토 리포트 (ISMS-P)</title>
<style>
...
<body>
  ...
  <h1>🛡️ Azure 정책 오프라인 보안검토 리포트</h1>
  <div class="meta">기준: ISMS-P 클라우드 인프라 통제항목 · 오프라인 규칙 기반 자동 검토</div>
  <div class="scorebox"><span class="score">{d['score']}</span> / 100점 &nbsp; 등급 <b>{_esc(d['grade'])}</b></div>
  <div class="meta">입력형식: {_esc(d['input_kind'])} · 파싱 리소스 {d['parsed_resources']}개 · 발견 이슈 {d['total_findings']}건 ({_esc(csum)})</div>
  {body_sections}
